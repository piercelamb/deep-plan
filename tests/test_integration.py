...
class TestPluginStructure:
    ...
    def test_skill_preflight_path_supports_hybrid_lookup(self, plugin_root):
        """SKILL preflight path discovery should use resolve_script_path function with CLAUDE_PLUGIN_ROOT priority."""
        skill_file = plugin_root / "skills" / "deep-plan" / "SKILL.md"
        ...
        # Check for CLAUDE_PLUGIN_ROOT priority (Strategy 1)
        assert 'CLAUDE_PLUGIN_ROOT' in content
        assert 'local candidate="$CLAUDE_PLUGIN_ROOT/$script_rel_path"' in content
        
        # Check for pwd fallback (Strategy 2)
        assert 'find "$(pwd)" -path "*/$script_rel_path"' in content
        
        # Check for usage
        assert 'resolve_script_path "scripts/checks/validate-env.sh"' in content
        
        # Check for helpful error messages
        assert 'plugin isn\'t properly installed or loaded' in content
